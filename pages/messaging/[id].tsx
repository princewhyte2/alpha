...
const Messaging = () => {
  ...
  const activeConversationParticipant = useMemo(() => {
    const chat: any = conversations?.find(
      (item: any) => item?.id == router.query?.id
    );
    ...
    try {
      if (router.query?.id) {
        ...
        const channel = pusher.subscribe(
          `private-chat-conversation-${router.query?.id}`
        );
    ...
    <Stack direction="column" sx={{ height: "100%" }}>
      ...
      >
        <Typography sx={{ fontSize: 16 }} color="primary.dark">
          {activeConversationParticipant?.first_name}{" "}
          {activeConversationParticipant?.last_name}
        </Typography>
          ...
          {conversation?.map((item: any) => {
            return (
              <Stack
                ...
                direction={"row"}
                justifyContent={
                  item.relationships?.sender?.id !== user?.id
                    ? "flex-start"
                    : "flex-end"
                }
              >
